...
(async function run() {
  ...
  if (!process.env.MONGO_URI) {
    console.error('ERROR: MONGO_URI not set in .env. Create .env from .env.example and set it.');
    process.exit(1);
  }

  try {
    const conn = await mongoose.connect(process.env.MONGO_URI, { dbName: process.env.MONGO_DBNAME || undefined });
    console.log('Mongoose connected. connection.name:', mongoose.connection.name);
    ...
    const engEmail = 'engineer@rotodyne.com';
